...
def arxiv_pdf_url(arxiv_id):
    """Return the direct arXiv PDF URL for an arXiv identifier."""
    if not arxiv_id:
        return None
    arxiv_id = str(arxiv_id).strip()
    # Keep versioned IDs valid, but remove accidental URL wrappers if present.
...
def clean_pdf_text(text):
    """Clean extracted PDF text while preserving rough paragraph/section breaks."""
    if not text:
        return ""
...
def extract_pdf_text(pdf_path, max_pages=50):
    """Extract text from a PDF. Uses PyMuPDF if available, otherwise pypdf."""
    try:
        import fitz  # PyMuPDF
...
def extract_relevant_fulltext_snippets(text, question="", max_snippets=8, window_chars=1800):
    """Extract targeted snippets likely to contain numerical values / methods.

    This is deliberately generic: it searches for common scientific-method and
    numerical-parameter words, plus terms from the actual question. The aim is
    to pass the useful parts of the paper to the agents, not just the first N
    ...
    lowered = text.lower()
    question_terms = [t for t in re.findall(r"[a-zA-Z0-9+()\-./]+", question.lower()) if len(t) >= 3]

    anchors = [
        # section-like anchors
        "computational details", "computational methods", "methods", "methodology",
        "simulation details", "calculation details", "theoretical methods",
        # DFT / electronic-structure anchors
        "density functional", "dft", "ab initio", "first-principles", "first principles",
        "quantum espresso", "vasp", "cp2k", "castep", "aims", "siesta",
        "pbe", "gga", "lda", "vdw", "dispersion", "d3", "d4", "optb", "rpa",
        "ccsd", "ccsd(t)", "diffusion monte carlo", "dmc", "qmc", "sapt",
        # numerical setup anchors
        "cutoff", "cut-off", "plane-wave", "plane wave", "k-point", "kpoint",
        "monkhorst", "vacuum", "supercell", "slab", "convergence", "pseudopotential",
        "basis set", "force convergence", "energy convergence", "geometry optimization",
        "relaxation", "adsorption energy", "binding energy", "interaction energy",
        # units / values likely to matter
        " ev", " mev", " kj/mol", " kcal/mol", " angstrom", " å", " bohr", " hartree",
    ]
    anchors += question_terms

    hits = []
    for anchor in compact_unique(anchors):
        a = anchor.lower()
        start = 0
        while True:
            ...
            if idx == -1:
                break
            hits.append((idx, anchor))
            start = idx + max(1, len(a))
            if len(hits) > 250:
                break
        if len(hits) > 250:
            break
    ...
    if not hits:
        return text[:max_snippets * window_chars]

    # Prefer earlier hits but remove overlapping windows.
    ...
    for idx, anchor in hits:
        ...
        if any(not (hi < ulo or lo > uhi) for ulo, uhi in used_ranges):
            continue
        snippet = text[lo:hi].strip()
        snippet = re.sub(r"\n{3,}", "\n\n", snippet)
        snippets.append(f"[snippet around: {anchor}]\n{snippet}")
...
def download_and_extract_arxiv_fulltext(paper, timeout=30, max_pages=50, max_chars=200000):
    """Download an arXiv PDF for a paper and attach an extracted text excerpt."""
    ...
    try:
        ...
        if not response.ok or not response.content.startswith(b"%PDF"):
            paper["full_text_status"] = f"arXiv PDF fetch failed: HTTP {response.status_code}"
            return ""

        with tempfile.NamedTemporaryFile(suffix=".pdf", delete=True) as f:
            f.write(response.content)
            f.flush()
            text = extract_pdf_text(f.name, max_pages=max_pages)

        if text:
            paper["full_text_status"] = f"arXiv PDF extracted ({len(text)} chars before truncation)"
            return text[:max_chars]

        paper["full_text_status"] = "arXiv PDF downloaded but no text extracted"
        return ""

    except Exception as exc:
        paper["full_text_status"] = f"arXiv full-text error: {exc}"
        return ""


def attach_arxiv_full_text(papers, question="", max_pages=50, max_chars=200000, methods_chars=20000):
    """Attach arXiv full text and targeted methods/value snippets to retained papers."""
    for p in papers:
        if p.get("arxiv"):
            print(f"Extracting arXiv full text: {p.get('title') or p.get('arxiv')}")
            full_text = download_and_extract_arxiv_fulltext(
                p,
                max_pages=max_pages,
                max_chars=max_chars,
            )
            p["full_text_excerpt"] = full_text
            snippets = extract_relevant_fulltext_snippets(full_text, question=question)
            p["methods_excerpt"] = snippets[:methods_chars]
        else:
            p["full_text_status"] = "No arXiv ID"
            p["full_text_excerpt"] = ""
            p["methods_excerpt"] = ""
    return papers
...
def papers_to_markdown(papers):
    ...
    for i, p in enumerate(papers, 1):
        ...
        blocks.append(
            ...
            f"Full-text status: {p.get('full_text_status') or 'Not attempted'}\n"
            f"Targeted methods/value snippets from full text: {(p.get('methods_excerpt') or 'No targeted methods/value snippets extracted.')}\n"
            f"Full-text excerpt: {(p.get('full_text_excerpt') or 'No full text extracted.')[:12000]}\n"
        )
    return "\n\n---\n\n".join(blocks)
...
- Use the targeted methods/value snippets from full text whenever available; abstracts alone are usually insufficient for computational settings.
...
- Use the targeted methods/value snippets from full text whenever available; abstracts alone are usually insufficient for values hidden in the paper body.
...
def paper_cards_html(papers):
    ...
    for i, p in enumerate(papers, 1):
        ...
        full_text_status = html.escape(p.get("full_text_status") or "Not attempted")
        ...
        <div class="paper-card" id="paper-{i}">
            ...
            <div class="paper-meta">full text: {full_text_status}</div>
...
def main():
    ...
    parser.add_argument("--no-arxiv-full-text", action="store_true", help="Disable arXiv PDF full-text extraction.")
    parser.add_argument("--full-text-max-pages", type=int, default=50, help="Maximum pages to extract from each arXiv PDF.")
    parser.add_argument("--full-text-max-chars", type=int, default=200000, help="Maximum extracted characters kept per arXiv paper.")
    parser.add_argument("--methods-snippet-chars", type=int, default=20000, help="Maximum targeted methods/value snippet characters kept per arXiv paper.")
    ...
    if not args.no_arxiv_full_text:
        relevant = attach_arxiv_full_text(
            relevant,
            question=question,
            max_pages=args.full_text_max_pages,
            max_chars=args.full_text_max_chars,
            methods_chars=args.methods_snippet_chars,
        )

    evidence_md = papers_to_markdown(relevant)
    full_text_md = "\n\n---\n\n".join(
        f"### Paper {i}\nTitle: {p.get('title') or 'Unknown'}\nStatus: {p.get('full_text_status') or 'Not attempted'}\n\n## Targeted methods/value snippets\n{p.get('methods_excerpt') or 'No targeted methods/value snippets extracted.'}\n\n## Full text excerpt\n{p.get('full_text_excerpt') or 'No full text extracted.'}"
    ...
    (output_dir / "full_text_excerpts.md").write_text(full_text_md, encoding="utf-8")
...
- Prioritize the targeted methods/value snippets from full text over the abstract when extracting computational parameters or numerical values.
- If a numerical value appears in the full-text snippet, cite the corresponding [Paper N](#paper-N).
"""
...
"""
    ...
    print(f"Full text excerpts: {output_dir / 'full_text_excerpts.md'}")
